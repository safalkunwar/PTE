...
import {
  ...
  DollarSign, ShoppingCart, AlertCircle,
} from "lucide-react";
...
export default function AdminDashboard() {
  ...
  const stats = {
    ...
    activeSubscriptions: systemStats?.activeSubscriptions ?? 0,
  ...
  const revenueByGateway = [
    { gateway: "Khalti", amount: 0, percentage: 0 },
    { gateway: "eSewa", amount: 0, percentage: 0 },
  ];

  const subscriptionPlans = [
    { name: "Free", users: stats.totalUsers, revenue: 0 },
  ];

  const recentPayments: { id: number; user: string; amount: number; gateway: string; date: string; status: string }[] = [];
              ...
              <CardContent>
                <div className="space-y-2">
                  {recentPayments.length === 0 ? (
                    <p className="text-slate-400 text-sm py-4">No payments yet</p>
                  ) : (
